...
class fdmnode{ /* contain the whole node used in one simulation */
    ...
    public:
        ...
        bool AdvanceInterTemp(const local_data_t * data, const bodyphysics & bPara, const double & dt, const unsigned int & iNplayer, const unsigned int & nDelta_all);
        bool AdvanceBoundaryTemp(const local_data_t * data, const bodyphysics & bPara, const double & dt_current, const unsigned int & iNplayer, const unsigned int & nDelta_all);
#ifdef USE_GAS
        ...
        double & Conductivity() {return dKappa;}
        double Conductivity() const {return dKappa;}
        ...
        vector3d get_scaled_vPos(){return scaled_vPos;}
        ...
        //accessors for lookup table population
        unsigned int get_num_neighbors() const { return (_nTYPE > 1) ? sNode.iNnode_s : 0; }
        unsigned int get_neighbor_id(unsigned int idx) const { 
            if (_nTYPE > 1 && idx < sNode.iNnode_s) {
                return sNode._nIDs[idx];
            }
            return 0;
        }
};
