...
#pragma omp parallel for schedule(static,nChunk)
        ...
        /* advance temperature */
        //loop over local map
        if (g_data && !g_data->local_nodes.empty()) {
            for (auto &kv : g_data->local_nodes) {
                fdmnode *node = kv.second;
                if (!node) continue;

                if (node->type() > 0) { /* interior node */
                    if (node->AdvanceInterTemp(g_data, bPara, _dDelta, iNplayer, _nDelta_all)) {
                        cout << "Error occurred while integrate the temperature of internal node No." << node->nID() << endl;
                        flag_error = true;
                    }
                }
                else { /* boundary node */
                    if (node->AdvanceBoundaryTemp(g_data, bPara, dTimeCurrent, iNplayer, _nDelta_all)) {
                        cout << "Error occurred while integrate the temperature of boundary node No." << node->nID() << endl;
                        flag_error = true;
                    }
                }
            }
        ...
        if (g_data && !g_data->local_nodes.empty()) {
            for (auto &kv : g_data->local_nodes) {
                fdmnode *node = kv.second;
                if (!node) continue;
                node->UpdateTemp(NodeVec, bPara, iNplayer);
            }
        } else {exit(1);}
    }
...
void Master::Output() {
    ...
    for (auto &kv : g_data->local_nodes) {
        fdmnode *node = kv.second;
        if (!node) continue;
        ftemp = (double) node->Temp();
        foutput.write( (char*) &ftemp, sizeof(double) );
        for ( i_vol=0; i_vol<NVOLATILE; i_vol++ ) {
            ftemp = (double) node->Rho_gas(i_vol);
            foutput.write( (char*) &ftemp, sizeof(double) );
            ftemp = (double) node->f_ice(i_vol);
            foutput.write( (char*) &ftemp, sizeof(double) );
        }
    }
    foutput.close();
