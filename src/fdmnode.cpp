...
double get_temp(const local_data_t * data, unsigned int nid){
    if(!data) exit(1);

    // first check if this node is local
    auto it_local = data->local_nodes.find(nid);
    if (it_local != data->local_nodes.end() && it_local->second) {
        return it_local->second->Temp();
    }

    // otherwise look it up in the global (remote) map
    auto it_global = data->global_data.find(nid);
    if (it_global != data->global_data.end() && it_global->second.temp) {
        return *(it_global->second.temp);
    }

    // if we get here something is inconsistent
    std::fprintf(stderr, "ERROR: get_temp could not find nid=%u\n", nid);
    std::fflush(stderr);
    std::exit(1);
}

double get_cond(const local_data_t * data, unsigned int nid){
    if(!data) exit(1);

    // first check if this node is local
    auto it_local = data->local_nodes.find(nid);
    if (it_local != data->local_nodes.end() && it_local->second) {
        return it_local->second->Conductivity();
    }

    // otherwise look it up in the global (remote) map
    auto it_global = data->global_data.find(nid);
    if (it_global != data->global_data.end() && it_global->second.cond) {
        return *(it_global->second.cond);
    }

    // if we get here something is inconsistent
    std::fprintf(stderr, "ERROR: get_cond could not find nid=%u\n", nid);
    std::fflush(stderr);
    std::exit(1);
}
...
bool fdmnode::AdvanceInterTemp(const local_data_t * data, const bodyphysics & bPara, const double & dt, const unsigned int & iNplayer, const unsigned int & nDelta_all ) {
    ...
    if ( _nTYPE > 1 ) { /* 3D GFDM node */
        ...
        mT(0) = dT_current;
        for ( i=0; i<sNode.iNnode_s; i++ ) {
            mT(i+1) = get_temp(data, sNode._nIDs[i]);
            mKappa(i+1) = get_cond(data, sNode._nIDs[i]);
        }
        mD = sNode.E.rows(0,5) * mT;
        ...
        dKappa_upper = 0.5 * ( dKappa + get_cond(data, _nID-iNplayer));
        dKappa_lower = 0.5 * ( dKappa + get_cond(data, _nID+iNplayer));
        iNumLayer = _nID/iNplayer;
        dA_self = bPara.dArea_1Dlayer[iNumLayer];
        dA_upper = 0.5 * ( dA_self + bPara.dArea_1Dlayer[iNumLayer-1] );
        dA_lower = 0.5 * ( dA_self + bPara.dArea_1Dlayer[iNumLayer+1] );
        
        dTrr = 2.0 * (dr_upper*dKappa_lower*dA_lower*( get_temp(data, _nID+iNplayer) - dT_current ) - dr_lower*dKappa_upper*dA_upper*( dT_current - get_temp(data, _nID-iNplayer))) / (dA_self*dr_upper*dr_lower*(dr_upper+dr_lower));
...
#ifdef USE_GAS
        double dTr;
        dTr = ( get_temp - fnvec[_nID-iNplayer].dT_current ) / ( dr_upper + dr_lower );
        for ( i_vol=0; i_vol<NVOLATILE; i_vol++ ) {
...
bool fdmnode::AdvanceBoundaryTemp(const local_data_t * data, const bodyphysics & bPara, const double & dt_current, const unsigned int & iNplayer, const unsigned int & nDelta_all) {
    unsigned int j;
    double cos_illumination0, cos_illumination, dT_solution, dKappa_mean;
    arma::vec P(5);
    
    dKappa_mean = 0.5 * ( dKappa + get_cond(data, _nID+iNplayer));
...
#else
    P(4) = - dKappa_mean / dr_lower * get_temp(data, _nID+iNplayer) - (1.0 - bPara.dA_B) * F_sun / bPara.oEle.drt_sq_AU * cos_illumination;
#endif
